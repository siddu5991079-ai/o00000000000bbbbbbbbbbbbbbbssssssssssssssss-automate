...
// =========================================================================
// 🔄 URL LOGIC FOR MULTIPLE SERVERS
// =========================================================================
...
// =========================================================================
// 🛡️ BACKGROUND SILENT LOADER (Fixes Black Screen Issue)
// =========================================================================
function loadBackupSilently(url) {
    console.log(`[*] Silently preparing Backup Page: ${url}`);
    
    // Naye tab ko background mein load karo
    backupPage.goto(url, { waitUntil: 'domcontentloaded', timeout: 60000 })
        .then(() => initializeVideo(backupPage, true, false))
        .catch(() => {})
        .finally(() => {
            if (activePage && !activePage.isClosed()) activePage.bringToFront().catch(()=>{});
        });

    // 🌟 Xvfb Focus Steal Protection: 
    // Tab load hote waqt screen black na ho, isliye hum baar baar Active ko samne layenge
    [200, 1000, 3000].forEach(ms => {
        setTimeout(() => { if (activePage && !activePage.isClosed()) activePage.bringToFront().catch(()=>{}); }, ms);
    });
}

// =========================================================================
// 🔄 SMART DUAL-TAB WATCHDOG 
// =========================================================================
...
async function startWatchdog() {
    ...
    while (true) {
        ...
        // 🌟 Zaroori Fix: Har 5 second baad Xvfb ko active tab dikhane par majboor karo
        if (activePage && !activePage.isClosed()) await activePage.bringToFront().catch(()=>{});

        let activeStatus = await checkPageStatus(activePage);
        let backupStatus = await checkPageStatus(backupPage);

        // 1. ACTIVE TAB HEALTH CHECK
        ...
        // 2. BACKGROUND BACKUP ROTATION
        if (activeStatus.status === 'HEALTHY' && (backupStatus.status === 'DEAD' || backupStatus.status === 'CRITICAL_ERROR' || backupStatus.status === 'FROZEN')) {
            console.log(`[!] Background Backup Tab failed (${backupStatus.status}). Quietly rotating to the NEXT server...`);
            backupUrlIndex = (backupUrlIndex + 1) % urlList.length; 
            loadBackupSilently(urlList[backupUrlIndex]);
        }

        // 3. ACTIVE TAB FAILURE -> HOT SWAP TRIGGER
...
async function startDirectStreaming() {
    ...
    console.log(`[*] STEP 2: Triggering Backup Page in the background...`);
    // Ab yeh process screen black nahi karega
    loadBackupSilently(urlList[backupUrlIndex]);
...
// // =========================================================================
// // 🔄 SMART DUAL-TAB WATCHDOG (INDEPENDENT MONITORING)
// // =========================================================================
// async function checkPageStatus(page) {
//     try {
//         for (const frame of page.frames()) {
...
//             console.log(`[*] Silently loading NEXT Server in background: ${urlList[backupUrlIndex]}`);
